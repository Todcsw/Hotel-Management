				...
				if (zh.d == 0) {
					...
					if (emp_name.equals(zh.zhyg(emp_name).getEmp_name())
						...
						try {
							...
							String str = emp_name + "在" + d1 + "时间找回密码";
						...
						dlpassword.setText("找会失败,用户名或密保答案   回答错误");
						Date d1 = new Date();
						try {
							String txtname = "lib/IO流记录系统的操作日志/员工操作日志/" + emp_name + "员工操作日志" + ".txt";
							File file = new File(txtname);
							...
							FileOutputStream out = new FileOutputStream(file, true);
							String str = emp_name + "在" + d1 + "时间找回密码,但找回失败";
							out.write(str.getBytes());
							out.write("\n".getBytes());
							out.close();
						} catch (IOException e2) {
